...
export default function Risorsa() {
    ...
    return(
        <main className="flex flex-col items-center justify-center w-full text-white mt-10">
            <section className="max-w-screen-2xl w-full center px-3.5 md:px-4 lg:px-6 xl:px-8 flex flex-col justify-start items-center mt-5">
                <h1 className="text-white text-center text-4xl font-bold">Nuova Risorsa</h1>
                <form
                    id="form"
                    className="flex flex-col gap-3 mt-5 justify-center"
                    onSubmit={Submit}>

                    <label className="flex flex-col justify-center gap-1" htmlFor="Titolo">Quale risorsa vuoi aggiungere?<input className="px-4 py-2 rounded-md text-sm font-medium bg-black border border-white/20" type="text" name="Titolo" placeholder="Titolo Risorsa" required /></label>
                    <label className="flex flex-col justify-center gap-1" htmlFor="Anno">
                        Di quale anno si tratta?
                        <select name="Anno" id="" className="bg-black text-sm rounded-md border border-white/20 font-medium px-2 py-px w-fit" required>
                    ...
                    <label htmlFor="Links" className="flex flex-col justify-center gap-1">
                        Links risorsa
                        <textarea className="bg-black border border-white/20 px-4 py-2 text-sm" name="Links" id=""></textarea>
                    </label>
                    <label htmlFor="Altro" className="flex flex-col justify-center gap-1">
                        Altre informazioni
                        <textarea className="border border-white/20 bg-black px-4 py-2 text-sm" name="Altro" id=""></textarea>
                    </label>
                    <div className="flex flex-row items-center gap-3">
                        <button id="submit-button" type="submit" className="bg-white text-black font-semibold w-fit px-4 py-1 rounded-md flex flex-row gap-1 items-center justify-center">Invia</button>
                        <svg className={`${submit == 2? "animate-spin inline": "hidden"} invert`} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
