...
export default function CardSubopen( {immagine, materia, descrizione, webex, materiale, registrazioni, prove, extraName, extra, extraLink, quantity} ) {
    ...
    if(!card) {
        display = (
            <div className="relative lg:w-6/12 w-full dark:bg-zinc-900 bg-zinc-200 lg:p-10 p-16 border rounded-2xl flex justify-center items-center border border-4 dark:border-zinc-700 border-zinc-400 dark:hover:border-zinc-500 hover:border-zinc-500 transition-all duration-300">
                ...
                <img rel="preload" src={immagine} className="border border-0 rounded-2xl lg:w-56 w-40 shadow-2xl dark:shadow-zinc-950 shadow-zinc-500"/>
            </div>
        )
    }
    else if(card){
        display = (
            <div className="relative lg:w-6/12 w-full dark:bg-zinc-900 bg-zinc-200 dark:border-zinc-500 border-zinc-400 dark:hover:border-zinc-400 hover:border-zinc-600 lg:p-10 px-7 py-9 border rounded-2xl flex border border-4 border-zinc-500 hover:border-zinc-500 transition-all duration-300">
                <svg onClick={() => useCard(!card)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-7 h-7 absolute right-2 top-2 dark:stroke-zinc-400 stroke-zinc-700 cursor-pointer">
    ...
    return(
        <>
            ...
            <div className="lg:w-6/12 w-full flex flex-col justify-center items-start">
                ...
                <a href={webex} className="dark:bg-white bg-black px-5 py-3 dark:text-zinc-900 text-zinc-100 font-bold text-lg border border-0 rounded-md mt-5 mb-3">Webex</a>
